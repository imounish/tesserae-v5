...
def latin_tokens(latin_files):
    ...
    tessfiles = sorted(filter(lambda x: os.path.splitext(x)[1] == ".tess", latin_files))
    ...
    for fname in tessfiles:
        ...
        for i in range(len(tokens)):
            if tokens[i]["TYPE"] == "WORD" and not re.search(
                r"[\d]", tokens[i]["DISPLAY"]
            ):
                text_tokens.append(
                    {
...
def test_init(token_connection):
    ...
    assert hasattr(t.lemmatizer, "lookup")
...
def test_normalize(token_connection, latin_tessfiles, latin_tokens):
    ...
    for i, tessfile in enumerate(latin_tessfiles):
        ...
        correct = map(lambda x: x[0] == x[1]["form"], zip(tokens, latin_tokens[i]))
...
def test_tokenize(token_connection, latin_tessfiles, latin_tokens):
    ...
    for i, tessfile in enumerate(latin_tessfiles):
        tokens, tags, features = la.tokenize(tessfile.read(), text=tessfile.metadata)
        ...
        for j, token in enumerate(tokens):
            ...
            assert all(
                [
                    any(map(lambda x: lemma.token == x, latin_tokens[i][j]["lemmata"]))
