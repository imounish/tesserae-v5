...
def greek_tokens(greek_files):
    ...
    tessfiles = sorted(filter(lambda x: os.path.splitext(x)[1] == ".tess", greek_files))
...
def test_normalize(token_connection, greek_tessfiles, greek_tokens):
    ...
    for i, tessfile in enumerate(greek_tessfiles):
        ...
        correct = map(lambda x: x[0] == x[1]["form"], zip(tokens, correct_tokens))
...
def test_tokenize(token_connection, greek_tessfiles, greek_tokens):
    ...
    for i, tessfile in enumerate(greek_tessfiles):
        ...
        tokens, tags, features = grc.tokenize(tessfile.read(), text=tessfile.metadata)
        ...
        for j, token in enumerate(tokens):
            ...
            assert all(
                [
                    any(map(lambda x: lemma.token == x, greek_tokens[i][j]["lemmata"]))
